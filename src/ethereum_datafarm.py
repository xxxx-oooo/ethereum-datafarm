import json
import random
from multiprocessing import Process, connection, cpu_count
...
class Contract:
    def __init__(self, address, name, method, startBlock, chunksize):
        ...
        newStartBlock, newStartTx = check_custom_start(
            self.name, self.simpleMethod
        )
        if newStartBlock:
            ...
            msg = "{} ({}) {}".format(
                self.address,
                self.name,
                colored("starting at last known location", "green"),
            )
            msg2 = "{} ({}) blockheight set to {:,.0f}".format(
                self.address, self.name, self.startBlock
            )
            print(INFO_MSG.format(msg))
            ...
            if newStartTx == "None":
                ...
                msg3 = "{} ({}) starting after tx {}".format(
                    self.address, self.name, self.startTx[:-56] + "..."
                )
        ...
        self.storageLocation = (
            f"../data/{self.name}/{self.simpleMethod}_" + "{}.csv"
        )
    ...
    def scrape(self):
        ...
        while self.fromblock < self.LATEST_BLOCK:
            ...
            while len(results) >= 1000:
                ...
                if results == "page limit reached":
                    ...
                    self.fromblock, self.startTx = check_custom_start(
                        self.name, self.simpleMethod
                    )
                    self.CACHE = pd.DataFrame(columns=self.columns)
                    self.run = False
            ...
            if (
                datetime.now() - self.timeSinceLatestBlock
            ).total_seconds() > 6e2:
        ...
        if len(self.CACHE) > 0 and self.run:
            ...
            dump_cache_to_disk(
                self.CACHE,
                self.storageLocation.format(self.fileCounter),
                self.name,
                self.simpleMethod,
            )
            self.CACHE = pd.DataFrame(columns=self.columns)
        ...
        with open(
            f"../tmp/{self.name}_{self.simpleMethod}_last_stored_tx.txt", "w"
        ) as f:
            f.write(content)
    ...
    def parse_results(self, results):
        for r in results:
            ...
            data = r["data"][2:]
            non_indexed_topics = list(
                abi_decode(self.evTypes, bytes.fromhex(data))
            )
    ...
    def log_progress(self, len_result, page):
        _printName = (
            self.printName[:16] + "..."
            if len(self.printName) >= 16
            else self.printName + "              "
        )
        msg = (
            "parsing {0:<19} | ".format(_printName[:19])
            + "{:>10,.0f}-{:>10,.0f} | ".format(self.fromblock, self.toblock)
            + colored("{:>4.0f}/1000".format(len_result), "green")
            + f" | Page {page}"
            + " | cs {:>7,.0f} | cache {:>6,.0f}".format(
                self.chunksize, len(self.CACHE)
            )
        )
    ...
    def log_chunk_size(self, old_size, op):
        ...
        msg = "{} chunk size for {}".format(
            op, self.printName[:17] + "...:"
        ) + " {:,.0f} --> {:,.0f} with avg. pages of {:.2f}".format(
            old_size, self.chunksize, avg
        )
        print(INFO_MSG.format(msg))
    ...
    def log_nothing_found(self):
        _printName = (
            self.printName[:14] + "..."
            if len(self.printName) >= 14
            else self.printName + "              "
        )
        msg = (
            colored("no result", "red")
            + " {0:<17} | ".format(colored(_printName[:17], "red"))
            + "{:>10,.0f}-{:>10,.0f} | {:>9}".format(
                self.fromblock,
                self.toblock,
                "cs " + "{:,.0f}".format(self.chunksize),
            )
        )
        print(INFO_MSG.format(msg))
    ...
    def log_storage(self):
        msg = colored(
            "storing ", "green"
        ) + f"{self.printName} with " "{:,.0f} entries @ ".format(
            len(self.CACHE)
        ) + colored(
            f"{self.storageLocation.format(self.fileCounter)}", "green"
        )
        print(INFO_MSG.format(msg))
        log(
            f"storing {self.printName} with {len(self.CACHE)} entries @ {self.storageLocation.format(self.fileCounter)}"
        )
