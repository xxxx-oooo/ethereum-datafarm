...
parser.add_argument(
    "-loc",
    "--location",
    help="output location - default: ../data",
    default="./data",
)
parser.add_argument(
    "-c", "--cores", help="cores available", default=str(cpu_count())
)
parser.add_argument(
    "-log", "--log", help="activate logging", action="store_true"
)
...
def get_method_from_canonical_expression(method):
    ...
    keccak_hash.update(method.encode("utf-8"))
    return f"0x{keccak_hash.hexdigest()}"
...
def latest_block():
    ...
    try:
        ...
        return (
            int(json.loads(res.content)["result"]) - 6
        )  # 6 blocks to make sure no re-orgs
    except:
...
def get_abi(contract):
    try:
        with open(f"../abis/{contract.name}.abi", "r") as file:
            abi = file.read()
            success = verify_abi(
                abi, contract.address, contract.name, contract.simpleMethod
            )

            if success:
                msg = (
                    "abi found locally | "
                    + colored("contract", "green")
                    + f" @ {contract.address} ({contract.name})"
                )
...
def set_up_directory(name, simpleMethod):
    ...
    if not os.path.isdir(f"../{LOCATION}/{name}"):
        ...
        lastFlNr = max(
            list(
                map(
                    int,
                    re.findall(
                        "[0-9]+", str(os.listdir(f"../{LOCATION}/{name}"))
                    ),
                )
            )
            + [0]
        ...
        print(
            INFO_MSG.format(
                f"fileCounter set to {fileCounter} for {name} and method {simpleMethod}"
            )
        )
...
def print_start():
    ...
    print(
        colored("© Toni Wahrstaetter 2023\n", "green", attrs=["dark", "bold"])
    )
